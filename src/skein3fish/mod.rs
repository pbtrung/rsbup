...
pub const SKEIN_HASH_LEN: usize = 64;
...
impl SkeinMAC {
    fn new(key: [u8; SKEIN_MAC_KEY_LEN]) -> SkeinMAC {
    ...
    fn final_mac(&mut self) -> [u8; SKEIN_MAC_LEN] {
        let mut output: [u8; SKEIN_MAC_LEN] = [0; SKEIN_MAC_LEN];
        unsafe {
            let rc: c_int;
            rc = skeinFinal(&mut self.skein_ctx, output.as_mut_ptr());
            if rc != 0 {
...
extern "C" {
    ...
    fn threefishEncryptBlockBytes(
        t3f_ctx: *const ThreefishKey,
        input: *const u8,
        output: *mut u8,
    );
    fn skeinCtxPrepare(
        skein_ctx: *mut SkeinCtx,
        skein_size: SkeinSize,
    ) -> c_int;
    fn skeinInit(skein_ctx: *mut SkeinCtx, hash_bit_len: size_t) -> c_int;
    fn skeinUpdate(
        skein_ctx: *mut SkeinCtx,
        input: *const u8,
        in_len: size_t,
    ) -> c_int;
    fn skeinReset(skein_ctx: *mut SkeinCtx);
...
pub fn hash(input: &[u8]) -> [u8; SKEIN_HASH_LEN] {
    let mut h: Skein_Ctxt_Hdr_t = Skein_Ctxt_Hdr_t {
        hash_bit_len: 0,
        b_cnt: 0,
        t: [0; SKEIN_MODIFIER_WORDS],
    };
    ...
    let mut output: [u8; SKEIN_HASH_LEN] = [0; SKEIN_HASH_LEN];
    unsafe {
        let mut rc: c_int;
        rc = skeinCtxPrepare(&mut skein_ctx, SkeinSize::Skein512);
        if rc != 0 {
        ...
        rc = skeinInit(&mut skein_ctx, SkeinSize::Skein512 as size_t);
        if rc != 0 {
            panic!("{}", "Error: skeinInit");
        }
...
pub fn mac(
    input: &[u8],
    skein_mac_key: &[u8; SKEIN_MAC_KEY_LEN],
) -> [u8; SKEIN_MAC_LEN] {
    let mut h: Skein_Ctxt_Hdr_t = Skein_Ctxt_Hdr_t {
        hash_bit_len: 0,
        b_cnt: 0,
...
fn test_block_encrypt() {
    ...
    let mut t3f = Threefish::new(&t3f_key, &t3f_tweak);
    let output = t3f.block_encrypt(&input);
}
...
fn test_mac() {
    ...
    let output = mac(&input, &skein_mac_key);
}
